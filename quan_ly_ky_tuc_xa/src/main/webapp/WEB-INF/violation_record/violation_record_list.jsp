...
<div class="container container-section" id="violation-section">
    ...
    <button class="btn btn-success mb-3" data-bs-toggle="modal" data-bs-target="#createContractModal" onclick="window.location.href='violationRecord?action=add'">
        <i class="fa fa-plus"></i> Create New Violation
    </button>
    <table class="table table-sm table-bordered text-center table-striped">
        <thead class="table-light">
        <tr>
            <th>ViolationRecordCode</th>
        ...
        <c:forEach var="violation" items="${violationRecords}">
            <tr>
                <td>${violation.violationRecordCode}</td>
                <td>${violation.violationStudentCode}</td>
